    ...
    <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 border border-white/20 h-full flex flex-col min-w-0">
      ...
      <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide flex-1 items-center min-w-0">
        {hourlyData.map((hour, index) => (
          ...
          >
            <div className="text-white/90 text-sm font-medium mb-2">{hour.time}</div>
            <div className="mb-2">
              <Image
            ...
            <div className="text-white font-semibold text-sm mb-1">{hour.temp}°</div>
            <div className="text-white/60 text-xs">{hour.feelsLike}°</div>
          </div>
        ))}
      </div>
    </div>
